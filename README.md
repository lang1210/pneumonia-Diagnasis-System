...
➡️ (https://pneumonia-diagnasis-system.streamlit.app/) ⬅️
...
使用resnet50+交叉熵损失 criterion = nn.CrossEntropyLoss()
...
对少数类的错判施加更大惩罚。结果有微小提升，但不足以扭转偏见。
...
用了方案2貌似没有变化（没改分类头，肯定没变化啊）

------------------------------

 ### 3. 对“正常”样本进行更强的数据增强 (Class-Specific Augmentation)
...
| **PNEUMONIA** | 0.8634 | 0.95649 | 0.9075 | 390 |
| **总准确率 (Accuracy)** | - | - | 0.8782 | 624 |
...
从混淆矩阵看出可以看出对Normal的召回率提升较大（虽然训练是计算代价也变大了）

------------------------------

### 4. 加权随机采样 (Weighted Random Sampling)
...
这个方法效果上和3差不多，但实际上只是改变了训练时样本抽取策略，计算量是较3小很多的
